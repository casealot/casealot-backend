...
//      refreshToken = authTokenProvider.convertAuthToken(customerRefreshToken.getRefreshToken());
      // refresh 토큰이 만료 됐을 경우 다시 재발급 한다.
...
//    CookieUtil.addCookie(response, REFRESH_TOKEN, refreshToken.getToken(), cookieMaxAge);
  ...
  public APIResponse<String> findId(CustomerIdFindDto customerIdFindDto) {
    Customer customer = customerRepository.findByNameAndEmail(customerIdFindDto.getName(),
        customerIdFindDto.getEmail());
    if (customer == null) {
      throw new NotFoundUserException();
    } else if (!customer.getName().equals(customerIdFindDto.getName())) {
      throw new CheckNameException();
    } else if (!customer.getEmail().equals(customerIdFindDto.getEmail())) {
      throw new CheckEmailException();
    }
    return APIResponse.success("id", customer.getId());
  }
